...
static int goodix_init_panel(struct goodix_ts_data *ts)
{
	...
	//This configuration used on both beitai and tianzhiyu TP.
	//config:GT811_Config_20130315_396kHz.cfg
	uint8_t config_info[] = {
	0x06,0xA2,
    ...
    0x02,0x00,0x07,0x77,0x16,0x66,0x26,0x66,\
    0x37,0x67,0x47,0x76,0x57,0x66,0x67,0x76,\
    0x77,0x77,0x86,0x77,0x97,0x78,0xA7,0x77,\
    0xB7,0x77,0xC7,0x76,0xD8,0x88,0xE9,0x99,\
    0xF4,0x44,0x07,0x03,0x80,0x80,0x80,0x19,\
    0x19,0x19,0x0F,0x0F,0x0A,0x40,0x22,0x0F,\
    0x03,0x00,0x05,0x58,0x02,0x00,0x04,0x00,\
    0x00,0x38,0x33,0x35,0x30,0x00,0x00,0x26,\
    0x20,0x02,0x08,0x00,0x00,0x00,0x00,0x00,\
    0x14,0x10,0x20,0x03,0x00,0x00,0x00,0x00,\
...
static void goodix_ts_work_func(struct work_struct *work)
{
    ...
    static u16 ignore_interrupt_number = 0;
    u8 report_num = 0;
    u8 touch_count = 0;
    ...
    //    goto exit_work_func;
    //}
    if(ignore_interrupt_number == 0)
    {
        ret = i2c_read_bytes(ts->client, point_data, sizeof(point_data)/sizeof(point_data[0]));
    }
    else
    {
	ignore_interrupt_number--;
	goto exit_work_func;
    }
    
    if (ret < 0) 
    {
	ignore_interrupt_number = 10;
        goto exit_work_func;
    }
    ...
    for(count=0,touch_count=0; count<report_num; count++)
    {
        if(point_index&(0x01<<count))
        {
			...
			if((input_y>490)&&(input_y<600))
			{
				point_tmp = (input_y-490)>>3;
				input_y -= point_tmp;
			}
