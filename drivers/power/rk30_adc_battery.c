...
/*******************ÒÔÏÂ²ÎÊý¿ÉÒÔÐÞžÄ******************************/
#define	TIMER_MS_COUNTS		 1000	//¶šÊ±Æ÷µÄ³€¶Èms
//ÒÔÏÂ²ÎÊýÐèÒªžùŸÝÊµŒÊ²âÊÔµ÷Õû
#define	SLOPE_SECOND_COUNTS	               15	//Í³ŒÆµçÑ¹Ð±ÂÊµÄÊ±ŒäŒäžôs
#define	DISCHARGE_MIN_SECOND	               45	//×î¿ì·Åµçµç1%Ê±Œä
#define	CHARGE_MIN_SECOND	               45	//×î¿ì³äµçµç1%Ê±Œä
#define	CHARGE_MID_SECOND	               90	//ÆÕÍš³äµçµç1%Ê±Œä
#define	CHARGE_MAX_SECOND	               250	//×î³€³äµçµç1%Ê±Œä
#define   CHARGE_FULL_DELAY_TIMES          10          //³äµçÂúŒì²â·À¶¶Ê±Œä
#define   USBCHARGE_IDENTIFY_TIMES        5           //²åÈëUSB»ìÁ÷£¬pcÊ¶±ðŒì²âÊ±Œä
...
#define   NUM_CHARGE_FULL_DELAY_TIMES         ((CHARGE_FULL_DELAY_TIMES * 1000) / TIMER_MS_COUNTS)	//³äµçÂú×ŽÌ¬³ÖÐøÊ±Œä³€¶È
#define   NUM_USBCHARGE_IDENTIFY_TIMES      ((USBCHARGE_IDENTIFY_TIMES * 1000) / TIMER_MS_COUNTS)	//³äµçÂú×ŽÌ¬³ÖÐøÊ±Œä³€¶È
...
//¶šÒåADC²ÉÑù·ÖÑ¹µç×è£¬ÒÔÊµŒÊÖµÎª×Œ£¬µ¥Î»K
...
//	struct rk30_adc_battery_platform_data *pdata = bat->pdata;
	...
	if (charge_level != bat->old_charge_level){
		...
		bat->bat_status_cnt = 0;        //×ŽÌ¬±ä»¯¿ªÊŒŒÆÊý
	}
...
//	struct rk30_adc_battery_platform_data *pdata = bat->pdata;
	...
	//³ä·Åµç×ŽÌ¬±ä»¯ºó£¬BufferÌîÂúÖ®Ç°£¬²»žüÐÂ
	...
	if (rk30_adc_battery_get_charge_level(bat)){
		if (capacity > bat->bat_capacity){
			...
			//ÊµŒÊ²ÉÑùµœµÄÈÝÁ¿±ÈÏÔÊŸµÄÈÝÁ¿Žó£¬ÖðŒ¶ÉÏÉý
		...
		else{  //   ÊµŒÊµÄÈÝÁ¿±È²ÉÑù±È ÏÔÊŸµÄÈÝÁ¿Ð¡
		            bat->gBatCapacityDisChargeCnt = 0;
			...
			//if (pdata->charge_ok_pin != INVALID_GPIO){
				...
				//Œì²âµœµç³Ø³äÂú±êÖŸ£¬Í¬Ê±³€Ê±ŒäÄÚ³äµçµçÑ¹ÎÞ±ä»¯£¬¿ªÊŒÆô¶¯ŒÆÊ±³äµç£¬¿ìËÙÉÏÉýÈÝÁ¿
					...
					//¹ý³ÌÖÐÈç¹ûµçÑ¹ÓÐÔö³€£¬¶šÊ±Æ÷žŽÎ»£¬·ÀÖ¹¶šÊ±Æ÷Ä£Äâ³äµç±ÈÊµŒÊ³äµç¿ì
			...
			//Ã»ÓÐ³äµçÂúŒì²âœÅ£¬³€Ê±ŒäÄÚµçÑ¹ÎÞ±ä»¯£¬¶šÊ±Æ÷Ä£Äâ³äµç
				if (capacity > bat->capacitytmp){
				//¹ý³ÌÖÐÈç¹ûµçÑ¹ÓÐÔö³€£¬¶šÊ±Æ÷žŽÎ»£¬·ÀÖ¹¶šÊ±Æ÷Ä£Äâ³äµç±ÈÊµŒÊ³äµç¿ì
					bat->gBatCapacityChargeCnt = 0;
				}
...
static void rk30_adc_battery_poweron_capacity_check(void)
{
	...
	//chargeing state
	//ÎÊÌâ£º
//	//1£©³€Ê±Œä¹Ø»ú·ÅÖÃºó£¬¿ª»úºó¶ÁÈ¡µÄÈÝÁ¿Ô¶Ô¶ŽóÓÚÊµŒÊÈÝÁ¿ÔõÃŽ°ì£¿
//	//2£©Èç¹û²»ÕâÑù×ö£¬¶ÌÊ±Œä¹Ø»úÔÙ¿ª»ú£¬Ç°ºóÈÝÁ¿²»Ò»ÖÂÓÖžÃÔõÃŽ°ì£¿
//	//3£©Ò»ÏÂÄÇÖÖ·œÊœºÏÊÊ£¿
...
#endif
/*******************************************
//¿ª»ú²ÉÑùµœµÄµçÑ¹ºÍÉÏŽÎ¹Ø»ú±£ŽæµçÑ¹Ïà²îœÏŽó£¬ÔõÃŽŽŠÀí£¿
